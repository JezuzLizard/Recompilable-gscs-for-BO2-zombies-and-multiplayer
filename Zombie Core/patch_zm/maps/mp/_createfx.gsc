...
draw_axis_think( axis_parent )
{
    ...
    for ( i = 0; 1; i = 0 )
    {
        asm_cond( !isdefined( axis_parent ), loc_48FC );
        asm_jump( loc_49CC );
        asm_cond( distancesquared( axis_model.origin, player.origin ) > range * range, loc_4940 );
        asm_cond( isdefined( axis_model ), loc_493D );
        axis_model delete();
        arrayremovevalue( level.draw_axis_models, undefined );
        asm_jump( loc_4980 );
        asm_cond( !isdefined( axis_model ), loc_4980 );
