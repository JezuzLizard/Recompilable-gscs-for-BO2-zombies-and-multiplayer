...
challengekills( data, time )
{
	...
	{
		if ( isDefined( player.secondaryloadoutweapon ) && weapon == player.secondaryloadoutweapon )
		{
...
#/
	...
	{
		_a930 = level.teams;
		_k930 = getFirstArrayKey( _a930 );
		while ( isDefined( _k930 ) )
		{
			team = _a930[ _k930 ];
			if ( team == player.team )
			{
			...
			{
				_k930 = getNextArrayKey( _a930, _k930 );
			}
		}
	}
...
{
	_a1485 = level.teams;
	_k1485 = getFirstArrayKey( _a1485 );
	while ( isDefined( _k1485 ) )
	{
		team = _a1485[ _k1485 ];
		if ( team == winner )
		{
		...
		_k1485 = getNextArrayKey( _a1485, _k1485 );
	}
	return 1;
}
...
killeddog()
{
	...
	while ( level.teambased )
	{
		...
		_a1712 = teammates.a;
		_k1712 = getFirstArrayKey( _a1712 );
		while ( isDefined( _k1712 ) )
		{
			player = _a1712[ _k1712 ];
			if ( player == self )
			{
			}
			else
			...
			{
				_k1712 = getNextArrayKey( _a1712, _k1712 );
			}
		}
	}
}
...
destroyedhelicopter( attacker, weapon, damagetype )
{
	...
	attacker addplayerstat( "destroy_helicopter", 1 );
}
