...
zone_init( zone_name )
{
	...
	while ( isDefined( zone.volumes[ 0 ].target ) )
	{
		...
		while ( i < spots.size )
		{
			...
			_a325 = tokens;
			_k325 = getFirstArrayKey( _a325 );
			while ( isDefined( _k325 ) )
			{
				token = _a325[ _k325 ];
				...
				_k325 = getNextArrayKey( _a325, _k325 );
			}
...
create_spawner_list( zkeys )
{
	...
	while ( z < zkeys.size )
	{
		...
		while ( zone.is_enabled && zone.is_active && zone.is_spawning_allowed )
		{
			...
			x = 0;
			while ( x < zone.astro_locations.size )
			{
			...
			x = 0;
			while ( x < zone.napalm_locations.size )
			{
			...
			x = 0;
			while ( x < zone.mechz_locations.size )
			{
