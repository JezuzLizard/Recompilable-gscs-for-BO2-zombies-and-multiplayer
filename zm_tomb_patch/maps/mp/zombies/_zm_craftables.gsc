...
is_point_in_craft_trigger( point )
{
	candidate_list = [];
	_a958 = level.zones;
	_k958 = getFirstArrayKey( _a958 );
	while ( isDefined( _k958 ) )
	{
		zone = _a958[ _k958 ];
		...
		_k958 = getNextArrayKey( _a958, _k958 );
	}
	valid_range = 128;
...
#/
	...
	craftablepickups = [];
	_a1320 = craftable.a_piecestubs;
	_k1320 = getFirstArrayKey( _a1320 );
	while ( isDefined( _k1320 ) )
	{
		piecestub = _a1320[ _k1320 ];
...
craftable_can_use_shared_piece()
{
	...
	{
		_a1614 = self.a_piecespawns;
		_k1614 = getFirstArrayKey( _a1614 );
		while ( isDefined( _k1614 ) )
		{
			piece = _a1614[ _k1614 ];
			...
			_k1614 = getNextArrayKey( _a1614, _k1614 );
		}
	}
	return 0;
}
...
craftable_set_piece_crafted( piecespawn_check, player )
{
	...
	_a1631 = craftablespawn_check.a_piecespawns;
	_k1631 = getFirstArrayKey( _a1631 );
	while ( isDefined( _k1631 ) )
	{
		piecespawn = _a1631[ _k1631 ];
		...
		_k1631 = getNextArrayKey( _a1631, _k1631 );
	}
}
...
{
	craftablespawn_check = get_actual_craftablespawn();
	_a1670 = craftablespawn_check.a_piecespawns;
	_k1670 = getFirstArrayKey( _a1670 );
	while ( isDefined( _k1670 ) )
	{
		piecespawn = _a1670[ _k1670 ];
		if ( isDefined( piecespawn_check ) )
		{
			if ( piecespawn.piecename == piecespawn_check.piecename && piecespawn.craftablename == piecespawn_check.craftablename )
			{
		...
		_k1670 = getNextArrayKey( _a1670, _k1670 );
	}
}
...
craftable_clear_piece_crafting( piecespawn_check )
{
	...
	craftablespawn_check = get_actual_craftablespawn();
	_a1699 = craftablespawn_check.a_piecespawns;
	_k1699 = getFirstArrayKey( _a1699 );
	while ( isDefined( _k1699 ) )
	{
		piecespawn = _a1699[ _k1699 ];
		if ( isDefined( piecespawn.is_shared ) && piecespawn.is_shared && isDefined( piecespawn.in_shared_inventory ) && piecespawn.in_shared_inventory )
		{
			piecespawn.crafting = 0;
		}
		_k1699 = getNextArrayKey( _a1699, _k1699 );
	}
}
...
{
	craftablespawn_check = get_actual_craftablespawn();
	_a1726 = craftablespawn_check.a_piecespawns;
	_k1726 = getFirstArrayKey( _a1726 );
	while ( isDefined( _k1726 ) )
	{
		piecespawn = _a1726[ _k1726 ];
		if ( isDefined( piecespawn_check ) )
		{
			if ( piecespawn.piecename == piecespawn_check.piecename && piecespawn.craftablename == piecespawn_check.craftablename )
			{
...
update_open_table_status()
{
	...
	_a1969 = level.a_uts_craftables;
	_k1969 = getFirstArrayKey( _a1969 );
	while ( isDefined( _k1969 ) )
	{
		uts_craftable = _a1969[ _k1969 ];
		...
		{
			b_piece_crafted = 0;
			_a1976 = uts_craftable.craftablespawn.a_piecespawns;
			_k1976 = getFirstArrayKey( _a1976 );
			while ( isDefined( _k1976 ) )
			{
				piecespawn = _a1976[ _k1976 ];
				...
				{
					_k1976 = getNextArrayKey( _a1976, _k1976 );
				}
			}
			if ( !b_piece_crafted )
			{
		...
		_k1969 = getNextArrayKey( _a1969, _k1969 );
	}
	while ( !b_open_craftables_remaining )
	{
		_a1995 = level.a_uts_craftables;
		_k1995 = getFirstArrayKey( _a1995 );
		while ( isDefined( _k1995 ) )
		{
			uts_craftable = _a1995[ _k1995 ];
			if ( uts_craftable.equipname == "open_table" )
			{
				thread maps/mp/zombies/_zm_unitrigger::unregister_unitrigger( uts_craftable );
			}
			_k1995 = getNextArrayKey( _a1995, _k1995 );
		}
	}
}
...
{
	_a2020 = level.a_uts_craftables;
	_k2020 = getFirstArrayKey( _a2020 );
	while ( isDefined( _k2020 ) )
	{
		uts_craftable = _a2020[ _k2020 ];
...
open_craftablestub_update_prompt( player )
{
	if ( isDefined( self.crafted ) && !self.crafted )
	{
		...
		_a2241 = level.a_uts_craftables;
		_k2241 = getFirstArrayKey( _a2241 );
		while ( isDefined( _k2241 ) )
		{
			uts_craftable = _a2241[ _k2241 ];
			...
			_k2241 = getNextArrayKey( _a2241, _k2241 );
		}
		...
		{
			if ( self.persistent == 1 || self.persistent == 2 && isDefined( self.bought ) && !self.bought )
			{
				return 1;
			}
		}
...
craftable_place_think()
{
	...
	else while ( !isDefined( player_crafted ) || self craftabletrigger_update_prompt( player_crafted ) )
	{
		...
		while ( self.stub.persistent == 1 )
		{
			...
			while ( isDefined( level.zombie_craftable_persistent_weapon ) )
			{
				while ( self [[ level.zombie_craftable_persistent_weapon ]]( player ) )
				{
					continue;
				}
			}
			while ( isDefined( level.zombie_custom_equipment_setup ) )
			{
				while ( self [[ level.zombie_custom_equipment_setup ]]( player ) )
				{
					continue;
				}
			}
...
{
	_a2775 = level.a_uts_craftables;
	_k2775 = getFirstArrayKey( _a2775 );
	while ( isDefined( _k2775 ) )
	{
		stub = _a2775[ _k2775 ];
		...
		_k2775 = getNextArrayKey( _a2775, _k2775 );
	}
	return undefined;
}
...
think_craftables()
{
	_a2873 = level.zombie_include_craftables;
	_k2873 = getFirstArrayKey( _a2873 );
	while ( isDefined( _k2873 ) )
	{
		craftable = _a2873[ _k2873 ];
		if ( isDefined( craftable.triggerthink ) )
		{
			craftable [[ craftable.triggerthink ]]();
		}
		_k2873 = getNextArrayKey( _a2873, _k2873 );
	}
}
...
opentablecraftable()
{
	...
	_a2887 = a_trigs;
	_k2887 = getFirstArrayKey( _a2887 );
	while ( isDefined( _k2887 ) )
	{
		trig = _a2887[ _k2887 ];
		setup_unitrigger_craftable_internal( trig, "open_table", "", "OPEN_CRAFTABLE", 1, 0 );
		_k2887 = getNextArrayKey( _a2887, _k2887 );
	}
}
...
is_holding_part( craftable_name, piece_name )
{
	...
	{
		_a3246 = level.a_uts_craftables;
		_k3246 = getFirstArrayKey( _a3246 );
		while ( isDefined( _k3246 ) )
		{
			craftable_stub = _a3246[ _k3246 ];
			while ( craftable_stub.craftablestub.name == craftable_name )
			{
				_a3251 = craftable_stub.craftablespawn.a_piecespawns;
			...
			_k3246 = getNextArrayKey( _a3246, _k3246 );
		}
	}
	return 0;
}
...
is_part_crafted( craftable_name, piece_name )
{
	while ( isDefined( level.a_uts_craftables ) )
	{
		_a3276 = level.a_uts_craftables;
		_k3276 = getFirstArrayKey( _a3276 );
		while ( isDefined( _k3276 ) )
		{
			craftable_stub = _a3276[ _k3276 ];
			while ( craftable_stub.craftablestub.name == craftable_name )
			{
				...
				_a3287 = craftable_stub.craftablespawn.a_piecespawns;
				_k3287 = getFirstArrayKey( _a3287 );
				while ( isDefined( _k3287 ) )
				{
					piece = _a3287[ _k3287 ];
					if ( piece.piecename == piece_name )
					{
					...
					_k3287 = getNextArrayKey( _a3287, _k3287 );
				}
			}
			_k3276 = getNextArrayKey( _a3276, _k3276 );
		}
	}
	return 0;
}
...
{
	_a3598 = level.a_uts_craftables;
	_k3598 = getFirstArrayKey( _a3598 );
	while ( isDefined( _k3598 ) )
	{
		uts_craftable = _a3598[ _k3598 ];
		...
		{
			_k3598 = getNextArrayKey( _a3598, _k3598 );
		}
	}
	return undefined;
}
...
{
	_a3618 = level.a_uts_craftables;
	_k3618 = getFirstArrayKey( _a3618 );
	while ( isDefined( _k3618 ) )
	{
		uts_craftable = _a3618[ _k3618 ];
		if ( uts_craftable.craftablestub.name == str_craftable )
		{
			_a3622 = uts_craftable.craftablespawn.a_piecespawns;
			_k3622 = getFirstArrayKey( _a3622 );
			while ( isDefined( _k3622 ) )
			{
				piecespawn = _a3622[ _k3622 ];
				if ( piecespawn.piecename == str_piece )
				{
					return piecespawn;
				}
				_k3622 = getNextArrayKey( _a3622, _k3622 );
			}
		}
		else _k3618 = getNextArrayKey( _a3618, _k3618 );
	}
	return undefined;
}
...
{
	_a3654 = level.a_uts_craftables;
	_k3654 = getFirstArrayKey( _a3654 );
	while ( isDefined( _k3654 ) )
	{
		uts_craftable = _a3654[ _k3654 ];
		if ( uts_craftable.craftablestub.name == str_craftable )
		{
			_a3658 = uts_craftable.craftablespawn.a_piecespawns;
			_k3658 = getFirstArrayKey( _a3658 );
			while ( isDefined( _k3658 ) )
			{
				piecespawn = _a3658[ _k3658 ];
				if ( piecespawn.piecename == str_piece && isDefined( piecespawn.model ) )
				{
					return piecespawn.model;
				}
				_k3658 = getNextArrayKey( _a3658, _k3658 );
			}
		}
		else _k3654 = getNextArrayKey( _a3654, _k3654 );
	}
	return undefined;
}
