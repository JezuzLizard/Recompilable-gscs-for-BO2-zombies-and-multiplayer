...
init_default_zcleansed_powerups()
{
	...
	maps/mp/zombies/_zm_powerups::add_zombie_powerup( "blue_monkey", "weapon_zombie_monkey_bomb", &"ZOMBIE_POWERUP_MAX_AMMO", ::maps/mp/zombies/_zm_powerups::func_should_never_drop, 1, 0, 0 );
}
...
allow_player_movement( allowed )
{
	...
	while ( isDefined( _k389 ) )
	{
		...
		{
			player freezecontrols( level.player_movement_suppressed );
		}
...
/#
	...
	{
		_a1261 = level._turned_zombie_respawnpoints;
		_k1261 = getFirstArrayKey( _a1261 );
		while ( isDefined( _k1261 ) )
		{
			spawnpoint = _a1261[ _k1261 ];
			text = "";
			color = ( 0, 0, 0 );
			...
			_k1261 = getNextArrayKey( _a1261, _k1261 );
#/
		}
	}
}
...
checkzombiehumanratio( playertomove, playertoignore )
{
	...
	players = get_players();
	_a1580 = players;
	_k1580 = getFirstArrayKey( _a1580 );
	while ( isDefined( _k1580 ) )
	{
		player = _a1580[ _k1580 ];
		...
		_k1580 = getNextArrayKey( _a1580, _k1580 );
	}
	if ( humancount > 1 )
	{
...
get_player_rank()
{
	...
	players = get_players();
	_a1650 = players;
	_k1650 = getFirstArrayKey( _a1650 );
	while ( isDefined( _k1650 ) )
	{
		player = _a1650[ _k1650 ];
		index = 0;
		...
		_k1650 = getNextArrayKey( _a1650, _k1650 );
	}
	index = 0;
...
award_round_end_bonus()
{
	...
	_a1762 = get_players();
	_k1762 = getFirstArrayKey( _a1762 );
	while ( isDefined( _k1762 ) )
	{
		player = _a1762[ _k1762 ];
		if ( player.score >= hiscore )
		{
		...
		_k1762 = getNextArrayKey( _a1762, _k1762 );
	}
}
...
zcleansed_zombie_powerup_grab( powerup, zombie_player )
{
	...
	switch( powerup.powerup_name )
	{
		case "the_cure":
			...
			zombie_player freezecontrols( 1 );
			zombie_player disappear_in_flash( 0 );
			zombie_player turn_to_human();
			players = get_players();
			_a1885 = players;
			_k1885 = getFirstArrayKey( _a1885 );
			while ( isDefined( _k1885 ) )
			{
				player = _a1885[ _k1885 ];
				if ( player.is_zombie )
				{
					player thread zombie_ramp_up();
				}
				_k1885 = getNextArrayKey( _a1885, _k1885 );
			}
			default:
...
init_cleansed_powerups()
{
	...
	add_cleansed_powerup( "green_monkey", "weapon_zombie_monkey_bomb", &"ZOMBIE_THIS_IS_A_BUG", 0, 0,4, 0, ::turned_powerup_green_monkey );
...
{
	total = 0;
	_a2062 = level.cleansed_powerups;
	_k2062 = getFirstArrayKey( _a2062 );
	while ( isDefined( _k2062 ) )
	{
		powerup = _a2062[ _k2062 ];
		powerup.recent = 0;
		i = 0;
		...
		_k2062 = getNextArrayKey( _a2062, _k2062 );
	}
	if ( total == 0 )
	{
		return undefined;
	...
	r = randomfloat( total );
	_a2081 = level.cleansed_powerups;
	_k2081 = getFirstArrayKey( _a2081 );
	while ( isDefined( _k2081 ) )
	{
		powerup = _a2081[ _k2081 ];
		if ( powerup.recent )
		{
		}
		else
		...
		_k2081 = getNextArrayKey( _a2081, _k2081 );
	}
	return undefined;
}
...
turned_powerup_green_nuke( player )
{
	...
	players = get_players();
	_a2219 = players;
	_k2219 = getFirstArrayKey( _a2219 );
	while ( isDefined( _k2219 ) )
	{
		target = _a2219[ _k2219 ];
		...
		_k2219 = getNextArrayKey( _a2219, _k2219 );
	}
}
...
turned_powerup_yellow_nuke( player )
{
	...
	players = get_players();
	_a2304 = players;
	_k2304 = getFirstArrayKey( _a2304 );
	while ( isDefined( _k2304 ) )
	{
		target = _a2304[ _k2304 ];
		if ( !cleansed_alive_check( target ) )
		{
		}
		else
		...
		_k2304 = getNextArrayKey( _a2304, _k2304 );
	}
}
